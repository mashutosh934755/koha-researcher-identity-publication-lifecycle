...
my $candidates = $dbh->selectall_arrayref(
    q{
        ...
        WHERE c.employment_status = 'active'
          ...
          AND (
                b.dateexpiry < CURDATE()
                OR COALESCE(b.lost, 0) <> 0
                OR COALESCE(b.gonenoaddress, 0) <> 0
          )
...
if (!@$candidates) {
    print "No exit candidates detected.\n";
    exit 0;
}
...
for my $row (@$candidates) {
    ...
    my $exists = $dbh->selectrow_array(
        q{
            SELECT COUNT(*)
            FROM researcher_audit_log
            WHERE borrowernumber = ?
              AND action_type =
                  'exit_candidate_detected'
              AND created_at >=
        ...
        $dbh->do(
            q{
                ...
                VALUES (
                    ...
                    'exit_candidate_detected',
                    ?,
                    'Patron account status requires manual no-dues or exit review',
                    'exit-watcher'
                )
            },
            undef,
...
print "No profile was automatically marked Former.\n";
